...
% Load CCF atlas
allen_atlas_path = '/Users/robinhaak/Downloads';
tv = readNPY([allen_atlas_path filesep 'template_volume_10um.npy']);
av = readNPY([allen_atlas_path filesep 'annotation_volume_10um_by_index.npy']);
st = loadStructureTree([allen_atlas_path filesep 'structure_tree_safe_2017.csv']);

% Set paths for histology images and directory to save slice/alignment
im_path = '/Users/robinhaak/Downloads/MockData';
...
resize_factor = 0.1; % (slides tiff: resize factor)
...
RH_manual_align_histology_ccf(tv,av,st,slice_path);
...
ccf_points = RH_histology2ccf(histology_points,slice_path);
...
ccf_count = cell(length(unique(ccf_points_av)), 2);

unique_areas = unique(ccf_points_av);
for i = 1:length(unique_areas) 
    ccf_count{i,1} = st(unique_areas(i),:).safe_name;
    ccf_count{i,2} = sum(ccf_points_av == unique_areas(i));
end
