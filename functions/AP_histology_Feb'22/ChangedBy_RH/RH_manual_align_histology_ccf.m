function RH_manual_align_histology_ccf(tv,av,st,slice_im_path)
	% AP_manual_align_histology_ccf(tv,av,st,slice_im_path)
	%
	% Align histology slices and matched CCF slices
	% Andy Peters (peters.andrew.j@gmail.com)
    
    %Robin Haak, Feb '22: affine changed to projective transform, save tform_manual
	...
	auto_ccf_alignment_fn = [slice_im_path filesep 'atlas2histology_tform_auto.mat'];
	...
	msgbox( ...
		...
		'\bf Controls: \rm' ...
		'1,2 : switch slice' ...
		'click : set reference points for manual alignment (4 minimum)', ...
	...
	switch eventdata.Key
		...
		case 'escape'
			...
			if strcmp(user_confirm,'Yes')
				atlas2histology_tform = ...
					gui_data.histology_ccf_manual_alignment;
				save_fn = [gui_data.slice_im_path filesep 'atlas2histology_tform_manual.mat'];
				save(save_fn,'atlas2histology_tform');
				disp(['Saved ' save_fn]);
				close(gui_fig);
			end
...
function mouseclick_histology(gui_fig,eventdata)
	...
	% If equal number of histology/atlas control points > 4, draw boundaries
...
function align_ccf_to_histology(gui_fig)
	...
	if size(gui_data.histology_control_points{gui_data.curr_slice},1) == ...
			...
			(size(gui_data.histology_control_points{gui_data.curr_slice},1) >= 4 && ...
			size(gui_data.atlas_control_points{gui_data.curr_slice},1) >= 4)
		% If same number of >= 4 control points, use control point alignment
		tform = fitgeotrans(gui_data.atlas_control_points{gui_data.curr_slice}, ...
			gui_data.histology_control_points{gui_data.curr_slice},'projective');
	...
	elseif size(gui_data.histology_control_points{gui_data.curr_slice},1) >= 1 ||  ...
		...
		% If less than 4 or nonmatching points, use auto but don't draw
		title(gui_data.histology_ax,'New alignment');
